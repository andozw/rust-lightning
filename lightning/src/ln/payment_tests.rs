...
fn retry_single_path_payment() {
	...
	let payment_id = nodes[0].node.send_payment(&route, payment_hash, &recipient_info).unwrap();
...
fn do_mpp_receive_timeout(send_partial_mpp: bool) {
	...
	// Initiate the MPP payment.
	let recipient_info = RecipientInfo { payment_secret: Some(payment_secret), payment_metadata: None };
	let _ = nodes[0].node.send_payment(&route, payment_hash, &recipient_info).unwrap();
	check_added_monitors!(nodes[0], 2); // one monitor per path
	let mut events = nodes[0].node.get_and_clear_pending_msg_events();
	assert_eq!(events.len(), 2);
...
fn no_pending_leak_on_initial_send_failure() {
	...
	let recipient_info = RecipientInfo { payment_secret: Some(payment_secret), payment_metadata: None };
	unwrap_send_err!(nodes[0].node.send_payment(&route, payment_hash, &recipient_info),
...
fn get_ldk_payment_preimage() {
	...
	let recipient_info = RecipientInfo { payment_secret: Some(payment_secret), payment_metadata: None };
	let _payment_id = nodes[0].node.send_payment(&route, payment_hash, &recipient_info).unwrap();
	check_added_monitors!(nodes[0], 1);
